import { isIE, stripUnwanted, internalPropertiesLang, hashString, internalProperties } from "./utils/general_utils";
...
export const TaskItem = function (pID, pName, pStart, pEnd, pClass, pLink, pMile, pRes, pComp, pGroup, pParent, pOpen,
  ...
  let vID = hashString(document.createTextNode(pID).data);
  ...
  if (parent && parent !== '0') {
    parent = hashString(parent).toString();
  }
  ...
  if (pDepend != null) {
    ...
    for (let k = 0; k < n; k++) {
      ...
      if (vDepend[k]) {
        vDepend[k] = hashString(vDepend[k]).toString();
      }
    }
  }
